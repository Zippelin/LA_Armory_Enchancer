...
export function getEngraveIconPathByNameFromLib(engraveName: string): string {
    const findedIndex: number = ENGRAVES_ICONS.findIndex(
        (element: EngraveIconPathType) => element.engrageName.trim().toLowerCase() == engraveName.trim().toLowerCase()
        )
    if (findedIndex >= 0) {
        const cdnIconPath = "efui_iconatlas/" + ENGRAVES_ICONS[findedIndex].iconName.split("_")[0] + '/' + ENGRAVES_ICONS[findedIndex].iconName
