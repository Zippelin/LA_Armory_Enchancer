...
function injectProfileEquipment(equip: {[key: string]: {[key: string]: {[key: string]: any}}}): void {
    for (let itemName in equip) {
        if (
            ...
            if (domElement) {
                domElement.insertAdjacentElement('beforeend', getProfileEqipmentItemTemplate(
                    equip[itemName].Element_001.value.qualityValue
                ))
            }
...
function injectGB() {
    ...
    const wrapper = document.querySelector<HTMLElement>('.profile-equipment__character')
    if (wrapper) {
